...
> 配合 `BRIEF.md`、`iterations/v1-launch/PRD.md` 和 `DESIGN.md` 阅读。这份文档描述首版的技术栈、数据策略、抓取筛选流程、部署方式和工程边界。

## 0. Manual Curation Mode

OpenAI is optional. The website, RSS feeds, and manual publishing flow only need
structured data under `data/news/`.

Manual mode:

```text
fixed source pool
-> fetch RSS candidates
-> human reads and verifies selected candidates
-> human writes data/manual/YYYY-MM-DD.json
...
## 0.1 Chinese Lead Discovery Mode
...
-> verified items become manual or automated publish drafts
```
...
## 0.2 Community Lead Submission Mode
...
-> verified leads become manual publish drafts
-> data/news/YYYY-MM-DD.json
```
...
- `verified`: eligible for the main timeline and RSS.
...
首版采用“静态内容站 + 定时抓取脚本 + 结构化数据 + RSS 生成”的架构。
...
Tailwind v4 用于快速落地 `DESIGN.md` 里的轻快日报视觉。设计 token 从 `DESIGN.md` 转成 CSS theme 后进入项目样式系统。避免复杂 JS 主题配置，优先使用 CSS-first 变量。

组件不追求“组件库感”。新闻条目、标签、RSS 复制块、日期分组、筛选按钮都用本地 Astro 组件实现。

## 3. 数据模型

首版不接数据库。所有已发布内容写入仓库内的结构化数据文件，构建时读取。
...
- 高置信候选写入 `data/news/YYYY-MM-DD.json`。
- 边界候选写入 `data/candidates/YYYY-MM-DD.json`。
- 明显不合格写入 `data/rejected/YYYY-MM-DD.json`，保留拒绝原因方便调试来源。

GitHub Actions 可以每天定时运行，并创建 Pull Request 或直接提交。首版建议先用 PR 模式运行 1-2 周，验证来源和 AI 筛选稳定后，再考虑高置信内容自动提交。
...
4. 去重
5. AI 结构化评分
6. 写入当天 news/candidates/rejected 数据
7. 创建 Pull Request 或提交到 main

首版建议 PR 模式，避免自动发布错误内容。
